...
- верхняя панель со статусом, simulation badge, HD toggle, debug toggle, guide overlay и кнопкой остановки;
- sensor strip с отдельными статусами `CAM`, `MIC`, `FACE`, `PULSE`, `HD`;
...
HD mode — это опциональный режим сегментации человека:

- включается отдельной кнопкой `HD`;
...
- при успешной загрузке переключает sensor strip в состояние `HD: active`.
